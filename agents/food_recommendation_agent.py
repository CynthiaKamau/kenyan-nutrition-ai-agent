...
from agents.patient_profile_agent import PatientProfileAgent
from agents.regional_food_agent import RegionalFoodAgent
...
class FoodRecommendationAgent:
    ...
    def _create_meal_plan(
        ...
        meal_plan["breakfast"]["proteins"] = [
            food for food in regional_foods.get("proteins", []) if food in ["eggs", "milk"]
        ][:1]
        meal_plan["breakfast"]["fruits"] = self._filter_by_gi(
            regional_foods.get("fruits", []),
            restrictions["limit_sugar"],
        )[:2]
        ...
        meal_plan["snacks"]["fruits"] = self._filter_by_gi(
            regional_foods.get("fruits", []),
            restrictions["limit_sugar"],
        )[:2]
        meal_plan["snacks"]["nuts"] = [
            food
            for food in regional_foods.get("legumes", [])
            if "nuts" in food or "groundnuts" in food
        ][:1]
    ...
    def _filter_by_gi(self, foods: List[str], limit_sugar: bool) -> List[str]:
        ...
        return low_gi_foods if low_gi_foods else foods[:2]

    def _get_preferred_foods(
        self,
        regional_foods: Dict[str, List[str]],
        restrictions: Dict[str, bool],
    ) -> Dict[str, List[str]]:
        ...
        if restrictions["increase_fiber"]:
            ...
            preferred["high_fiber"] = [
                food for food in high_fiber_foods if any(food in category for category in regional_foods.values())
            ]

        lean_proteins = ["fish", "chicken", "eggs"]
        preferred["lean_proteins"] = [
            food for food in lean_proteins if food in regional_foods.get("proteins", [])
        ]

        complex_carbs = ["millet", "sorghum", "sweet_potatoes"]
        preferred["complex_carbs"] = [
            food for food in complex_carbs if any(food in category for category in regional_foods.values())
        ]

        return preferred

    def _get_foods_to_limit(
        self,
        regional_foods: Dict[str, List[str]],
        restrictions: Dict[str, bool],
    ) -> Dict[str, List[str]]:
        ...
        if restrictions["limit_sugar"]:
            ...
            limit["high_gi_foods"] = [
                food for food in high_gi_foods if any(food in category for category in regional_foods.values())
            ]

        if restrictions["limit_saturated_fat"]:
            high_fat_foods = ["coconut_milk", "groundnuts"]
            limit["high_fat_foods"] = [
                food for food in high_fat_foods if any(food in category for category in regional_foods.values())
            ]
